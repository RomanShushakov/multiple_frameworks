<script>
    ...
    const showMessage = async (success, message) => {
        ...
        setTimeout(() => {
            ...
            errorMessage = "";
            if (success) {
                getData(import.meta.env.VITE_INDEX_URL)
                    .then(response => {
    ...
    const handleSubmit = async () => {
        if (!validate(form)) {
            formErrors[validate.errors[0].instancePath.slice(1)] = validate.errors[0].message;
            setTimeout(() => {
        ...
        const [success, message, token] = await login(import.meta.env.VITE_LOGIN_URL, form);
...
<div class="wrapper">
    ...
    <div class="row justify-content-center">
        <span
            ...
            style="margin-bottom: 1rem; display: {successMessage || errorMessage ? 'block' : 'none'};"
        >
            {@html successMessage || errorMessage}
        </span>
    </div>

    <a href={import.meta.env.VITE_INDEX_URL} class="link-secondary">to the start page</a>
</div>
